...
export async function POST(
  ...
  try {
    ...
    if (train.status !== 'STOPPED') {
      return NextResponse.json({ error: 'القطار ليس في حالة توقف' }, { status: 409 })
    }

    const updated = await prisma.train.update({
      where: { id: params.id },
      data: { status: 'ACTIVE', speed: 40 },
    })

    return NextResponse.json({ message: 'تم استئناف القطار', train: updated })
  } catch {
    return NextResponse.json({ error: 'فشل استئناف القطار' }, { status: 500 })
  }
}
