...
import { STATIONS, SHIFTS, INCIDENT_TYPES, STAFF_ROLES } from "../types";
...
const defaultValues = {
  ...
  passengers: [] as { name: string; age: string; phone: string; emergency_contact: string; passenger_status: string; hospital_name: string }[],
  train_number: "", current_location: "", destination: "", operation_mode: "",
  rescue_train_number: "", rescue_start_time: "", rescue_end_time: "",
  ...
  staff: [] as { name: string; employee_id: string; role: string }[],
  incident_duration: "", response_duration: "", train_delays: "", passengers_affected: "",
...
export default function IncidentForm() {
  ...
  useEffect(() => {
    if (existing) {
      reset({
        ...
        staff: existing.staff.map((s) => ({ name: s.name || "", employee_id: s.employee_id || "", role: s.role || "" })),
  ...
  const onSubmit = (data: typeof defaultValues) => {
    const payload: Record<string, unknown> = {
      ...
      staff: data.staff.map((s) => ({ name: s.name || null, employee_id: s.employee_id || null, role: s.role || null })),
      impact: {
  ...
  const stepContent = (step: number) => {
    switch (step) {
      case 0:
        return (
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}><Controller name="location" control={control} render={({ field }) => <TextField {...field} fullWidth label="Location" />} /></Grid>
      ...
      case 1:
        return (
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6}><Controller name="discovered_by" control={control} render={({ field }) => <TextField {...field} fullWidth label="Discovered By" />} /></Grid>
            <Grid item xs={12} sm={6}><Controller name="first_reporter" control={control} render={({ field }) => <TextField {...field} fullWidth label="First Reporter" />} /></Grid>
      ...
      case 3: {
        ...
        return (
          <Box>
            ...
            <Button variant="outlined" onClick={() => setValue("passengers", [...passengers, { name: "", age: "", phone: "", emergency_contact: "", passenger_status: "", hospital_name: "" }])}>
              + Add Passenger
            </Button>
          </Box>
        );
      ...
      case 4:
        return (
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} sm={6}><Controller name="operation_mode" control={control} render={({ field }) => <TextField {...field} fullWidth label="Operation Mode" />} /></Grid>
      ...
      case 6: {
        ...
        return (
          <Box>
            ...
            <Button variant="outlined" onClick={() => setValue("staff", [...staff, { name: "", employee_id: "", role: "" }])}>
              + Add Staff Member
            </Button>
          </Box>
        );
      ...
      case 7:
        return (
          <Grid container spacing={2}>
            <Grid item xs={12} sm={4}><Controller name="incident_duration" control={control} render={({ field }) => <TextField {...field} fullWidth label="Incident Duration (min)" type="number" />} /></Grid>
            <Grid item xs={12} sm={4}><Controller name="response_duration" control={control} render={({ field }) => <TextField {...field} fullWidth label="Response Duration (min)" type="number" />} /></Grid>
            <Grid item xs={12} sm={4}><Controller name="train_delays" control={control} render={({ field }) => <TextField {...field} fullWidth label="Train Delays" type="number" />} /></Grid>
